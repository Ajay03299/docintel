...
from app.engines.understanding.extraction import (
    ExtractionService, NativeExtractor, OcrExtractor,
)
...
def process_document(self, document_id: str) -> dict:
    ...
    try:
        with session_scope() as db:
            ...
            if doc.status in (DocumentStatus.EXTRACTED, DocumentStatus.VALIDATED,
            ...
            structured = StructuredExtractor(provider, max_retries=1).extract(
                document_text=text_result.full_text,
                system_prompt=INVOICE_SYSTEM_PROMPT,
                user_prompt=build_invoice_prompt(text_result.full_text),
            ...
            log.info("pipeline.extracted", document_id=document_id,
                     method=text_result.method.value, parse_error=structured.parse_error)
            return {"status": "extracted", "parse_error": structured.parse_error}
        ...
        log.warning("pipeline.error", document_id=document_id, error=str(exc),
